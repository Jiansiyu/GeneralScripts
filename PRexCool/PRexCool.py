'''
    PRex slurm Replay Scripts
It could work locally or work on the ifarm
...
class runInfor(object):
    ...
    def __init__(self,runID=1000):
        pass

    def LoadConfig(self):
        pass

    def LoadRunInforList(self):
        pass
...
class ArgParser(object):
    ...
    def decodeRunFile(self,RunListFileName):
        '''
    ...
    def CheckReplayFileExist(self,runID):
        return True


    def DecodeRuns(self,argv, checkRunRawFileExits=True,checkReplayed=False):
        if len(argv) < 1:
            pass

        for runID in argv[1:]:
            if runID.isdigit():
                self.inputRunList.append(int(runID))
            else:
                if '.txt' in runID:
                    for runs in self.decodeRunFile(RunListFileName=runID):
                        self.inputRunList.append(int(runs))
                if '-' in runID:
                    ...
                    if runStart.isdigit() and runEnd.isdigit():
                        ...
                        while runStart <= runEnd:
                            self.inputRunList.append(runStart)
                            runStart=runStart+1
        if checkRunRawFileExits:
            for runID in self.inputRunList:
                if not self.CheckReplayFileExist(runID=runID):
                    self.inputRunList.remove(runID)

        return self.inputRunList

class PRexGooo(object):
    def __init__(self):
        pass
